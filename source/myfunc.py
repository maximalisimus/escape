...
class TGroup(TList):
	...
	def __init__(self, elem = None):
		super(TGroup, self).__init__(elem)
...
class TSprite:
	...
	def __init__(self, image = None, rect = None, ongroup = None):
		...
		if ongroup != None:
			ongroup.add(self)
			if ongroup not in groups:
				groups.append(ongroup)
	...
	def add(self, ongroup):
		global groups
		if self not in ongroup():
			ongroup.add(self)
		if ongroup not in groups:
			groups.append(ongroup)
	
	def remove(self, ongroup):
		global groups
		if self in ongroup:
			ongroup.remove(self)
